...
add_action( 'plugins_loaded', function() {
	...
	if (! Tamara_Checkout_Helper::check_enpii_base_plugin()) {
		...
		$error_message .= sprintf( __( 'Plugin <strong>%s</strong> is required.', Tamara_Checkout_Helper::TEXT_DOMAIN ), 'Enpii Base');
	}

	if (! Tamara_Checkout_Helper::check_woocommerce_plugin()) {
		$error_message .= $error_message ? '<br />' : '';
		$error_message .= sprintf( __( 'Plugin <strong>%s</strong> is required.', Tamara_Checkout_Helper::TEXT_DOMAIN ), 'WooCommerce');
	}

	if ($error_message) {
		add_action( 'admin_notices', function() use ($error_message) {
			$error_message = sprintf(
				__( 'Plugin <strong>%s</strong> is disabled.', Tamara_Checkout_Helper::TEXT_DOMAIN ),
				'Tamara Checkout'
			) . '<br />' . $error_message;
