...
class WC_Order_Woo7_Repository implements WC_Order_Repository_Contract {
	...
	public function get_stuck_authorised_wc_orders( int $page = 0, int $items_per_page = 20 ): array {
		...
		$processing_orders_query = Post::site( $this->site_id );
		$processing_orders_query = $processing_orders_query->select( [ 'posts.ID', 'posts.post_date_gmt', 'posts.post_status', 'postmeta.meta_value as payment_method' ] )
		->join(
			'postmeta',
			function ( JoinClause $join ) {
				$join->on( 'postmeta.post_id', '=', 'posts.ID' )
